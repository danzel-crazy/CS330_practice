...
class MultiTaskNet(nn.Module):
    ...
    def init_shared_user_and_item_embeddings(self, num_users, num_items, embedding_dim):
        ...
        U = Q = None
        ### START CODE HERE ###
        Scaled_layer = ScaledEmbedding(num_embeddings=num_users, embedding_dim=embedding_dim)
        # Scaled_layer.reset_parameters()
        U = Scaled_layer

        Scaled_layer_q = ScaledEmbedding(num_embeddings=num_items, embedding_dim=embedding_dim)
        # Scaled_layer_q.reset_parameters()
        Q = Scaled_layer_q
        ### END CODE HERE ###
        return U, Q
    ...
    def init_mlp_layers(self, layer_sizes):
        ...
        for fan_in, fan_out in zip(layer_sizes, layer_sizes[1:]):
            mlp_layers.append(nn.Linear(fan_in, fan_out))  # Append Linear layer
            mlp_layers.append(nn.ReLU())  # Append ReLU activation
    ...
    def forward_with_embedding_sharing(self, user_ids, item_ids):
        ...
        # Regression head
        user_embed = self.U(user_ids) # (batch, embed)
        item_embed = self.Q(item_ids) # (batch, embed)
        dot_product = self.U(user_ids) * self.Q(item_ids)
        
        score = torch.cat((user_embed, item_embed, dot_product), dim=1)
        
        for l in self.mlp_layers:
            score = l(score)

        score = score.squeeze(1) # (batch, 1) -> (batch,)


        # Matrix Factorization Head  
        predictions = ((user_embed * item_embed).sum(dim=1, keepdim=True) + self.B(item_ids)).squeeze(1) # (batch, 1) -> (batch,)
        ### END CODE HERE ###
        return predictions, score
    ...
    def forward_without_embedding_sharing(self, user_ids, item_ids):
        ...
        # Regression head
        user_embed = self.U_reg(user_ids)
        item_embed = self.Q_reg(item_ids)
        dot_product = self.U_reg(user_ids) * self.Q_reg(item_ids)
        score = torch.cat((user_embed, item_embed, dot_product), dim=1)
        ...
        # Matrix Factorization Head 
        user_embed_fact = self.U_fact(user_ids)
        item_embed_fact = self.Q_fact(item_ids)
        predictions = ((user_embed_fact * item_embed_fact).sum(dim=1, keepdim=True) + self.B(item_ids)).squeeze(1) # (batch, 1) -> (batch,)
        ### END CODE HERE ###
        return predictions, score